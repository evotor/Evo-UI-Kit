...
- **Ширины колонок не подстраиваются под контент.**
  ...
  По умолчанию колонки равной ширины; нужную ширину задавайте классом колонки через `className`.
  Суммарная ширина колонок должна помещаться в ширину таблицы.
  Горизонтальный скролл шапки и тела не синхронизирован: тело - это вьюпорт CDK со скроллом по обеим осям, а шапка вынесена из него с `overflow: hidden`.
  Если колонки в сумме шире таблицы, тело прокрутится вправо, а шапка останется на месте, и колонки шапки разъедутся с колонками строк по горизонтали.
  Задавайте ширины так, чтобы они укладывались в ширину таблицы.
...
- **Мобильная раскладка не применяется.**
  Стопка ячеек с подписями строк даёт переменную высоту, несовместимую с фиксированной `rowHeight`, поэтому раскладка всегда колоночная, подписей строк нет, а шапка видна на всех вьюпортах.
  Темы `evo-table_mobile`, `evo-table_mobile-align_right`, `evo-table_mobile_short`, `evo-table_with-title` в этом режиме не поддерживаются.
  Если мобильная раскладка нужна, гейтите сам вход: `[virtualScroll]="isDesktop"`.
