...
// across Karma's randomized, teardown-disabled suite (a stuck frame-handle left by any earlier spec
// later flushes against an emptied queue: `Cannot read property 'execute' of undefined`). The
// strategy lets specs swap in a scheduler, so we drive throttling with `asyncScheduler`: timer-based,
// advanced by `tick`, and free of the shared frame-handle that made these tests order-dependent.
