...
describe('EvoRepositionScrollStrategy', () => {
    ...
    // The reposition stream is rAF-throttled (auditTime(0, animationFrameScheduler)). zone.js'
    // fakeAsync on this Angular version does not flush RxJS' animationFrameScheduler, so requestAnimationFrame
    // is captured and driven manually: a same-frame burst queues a single callback, and flushFrames()
    // runs it deterministically.
    let frameCallbacks: FrameRequestCallback[];

    function flushFrames(): void {
        for (let guard = 0; guard < 10 && frameCallbacks.length; guard++) {
    ...
    describe('in a browser', () => {
        ...
        it('repositions when an inner container that is NOT an ancestor of the trigger scrolls', () => {
        ...
        it('coalesces a same-frame burst of scroll events to a single updatePosition', () => {
            const strategy = createStrategy();
            strategy.enable();
        ...
        it('still repositions on page/document scroll (handled by the CDK global listener)', () => {
            const strategy = createStrategy();
            strategy.enable();
        ...
        it('repositions on body scroll (body height:100dvh case)', () => {
            const strategy = createStrategy();
            strategy.enable();
        ...
        it('ignores scrolls that originate inside the overlay', () => {
            const strategy = createStrategy();
            strategy.enable();
    ...
    describe('without a browser platform (SSR)', () => {
        ...
        it('does not touch window or reposition on scroll', () => {
            const strategy = createStrategy();
    ...
    describe('closing when the trigger leaves its scroll container', () => {
        ...
        it('stays open while the trigger is still inside its container', () => {
        ...
        it('closes when the trigger has scrolled out of its container', () => {
            spyOn(scrollContainer, 'getBoundingClientRect').and.returnValue(rect(0, 100, 0, 100));
